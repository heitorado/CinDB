<?php
/* Movi para cima, geralmente quando vai iniciar as coisas em php
deixa aqui em cima, questao de organização*/
...
<body>
	...
	<!-- modal trigger-->
	<button type="button" class="btn btn-info btn-default" data-toggle="modal" data-target="#modalCadastro">
		Criar Nova
	</button>
	<hr>
	...
	<table id="dbsummary">
		...
		<?php
			$sql = "SELECT a.idcadeiras, a.nome, b.nome" .
				" FROM cadeiras a, tags b, cadeiras_has_tags c" .
				" WHERE a.idcadeiras = c.cadeiras_idcadeiras" .
				" AND b.idtags = c.tags_idtags;";
			...
			<tr>
				<td id='idcol'><?=$row["idcadeiras"]?></td>
				<td><?=$row["nome"][0]?></td>
				<td><?=$row["nome"][1]?></td>
				<td> vai ter professores aqui</td>
